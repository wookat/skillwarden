---
title: Comparison
description: How SkillWarden compares with Snyk Agent Scan, skillgate.sh, skill-tools, skill-check, and skillmds — source-verified.
---

> Last verified: 2026-08-04. Every claim below was checked against the competitor's
> public README / repository source code / npm registry metadata on that date (links
> in each section). Star counts are from the GitHub API on the same date and will
> drift. SkillWarden is under active development — rows not yet shipped are marked
> "planned". The canonical, maintained version of this document is
> [docs/COMPARISON.md](https://github.com/wookat/skillwarden/blob/main/docs/COMPARISON.md).
...
- **Naming consequence**: `skillgate` is taken and npm strips punctuation when
  checking for duplicates, so `skill-gate` would also be rejected; we renamed to
  SkillWarden and publish as `skillwarden` / `skillwarden-core` (all variants verified
  404 on the registry 2026-08-04), stating the non-affiliation prominently in the README.
