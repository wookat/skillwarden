...
- **Security vulnerabilities in SkillGate itself:** do NOT open a public issue — see
...
| Package | What it is |
|---|---|
| `packages/core` | rule engine + lockfile implementation (`skill-gate-core`) |
| `packages/cli` | the `skillgate` CLI (npm package `skill-gate`) |
...
5. Follow the existing code style (enforced by ESLint); no new runtime dependencies
   without discussion in an issue first (`skill-gate-core` is intentionally
   zero-dependency).
