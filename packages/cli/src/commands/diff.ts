...
import { buildLockfile, diffLockfiles, LOCKFILE_NAME, readLockfile } from 'skillwarden-core';
...
export function runDiff(paths: string[], options: DiffCommandOptions, cwd: string): number {
  ...
  if (!existsSync(lockPath)) {
    process.stderr.write(`No lockfile found at ${lockPath}. Run \`skillwarden lock\` first.\n`);
