...
- `benign-skill/` — a realistic, clean skill. `skillwarden scan examples/skills/benign-skill` exits 0.
- `malicious-skill/` — a deliberately malicious skill combining instruction override,
  ...
  obfuscated payload in a bundled script. `skillwarden scan examples/skills/malicious-skill` exits 1.

The malicious example is inert: the endpoints are not real dead drops and nothing here
is executed by the scanner — SkillWarden only reads files.
