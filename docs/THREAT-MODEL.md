...
| `exfiltration` | Network calls carrying env secrets, env dumps piped to network tools, env harvesting loops filtered by secret names, reads of `~/.ssh`/`~/.aws`/`~/.netrc`/`~/.npmrc` (shell **and** language-runtime forms), dead-drop services (webhook.site, requestbin, interactsh…), ephemeral tunnels (ngrok, trycloudflare), raw-IP URLs and bare public-IP endpoint constants, host/workspace fingerprint telemetry, disposable free-tier collectors | `rankaj` → `https://webhook.site/…` | Yes — `critical` dead-drop hit (verified locally 2026-08-04). Note the *file path* it read (`~/.clawdbot/.env`) was not itself flagged (§7 R5) |
